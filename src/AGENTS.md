...
- `FluidTelemetry` (in `fluid/mod.rs`) is the only audio→UI channel: the audio thread only ever *stores* (relaxed atomics, zero alloc, zero locks); the UI thread only ever *loads*. It carries per-voice trigger pulses (monotonic counters), last bass/tonal note Hz, smoothed per-voice output levels, chord index, and beat. Levels come from per-sample peak followers in `FluidEngine` published every `LEVEL_PUBLISH_INTERVAL` samples (control rate, never per sample). Telemetry is a pure side-channel — it must never influence the sample path, so `render --seed` output stays byte-identical. Add new signals by following the atomic-store pattern exactly.
- The `ui.rs` visualizer is a node system: one visual body per voice (`Node` for persistent bass/pad/perc/clap, `Wavelet` for transient kick/tonal/perc/clap triggers), superposed in `FluidState::field` into a per-cell `FieldSample { value, hue }`. Pitch→visual mappings (`bass_spatial_freq`, `tonal_node_y`) are pure and tested. Keep the per-cell field cost bounded: precompute per-node/per-frame terms in `tick`, cap live wavelets at `MAX_WAVELETS`. A silent voice's node must fall to zero amplitude (dark). The `V` key toggles a control-panel-hidden focus mode.
