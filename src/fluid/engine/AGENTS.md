...
- Bloom's return ramps out linearly over `BLOOM_RELEASE_SECONDS` (50 ms) once its envelope falls, and the dry duck returns with the envelope, so the wash stops rather than ringing on; a repress slews it back up. Echo retires after a bounded six-second tail with a smooth final fade. Both then clear incrementally, and a fresh press during clearing must accept new input.
...
- Verify audible changes and releases through rendered samples, including overlapping gestures. `bloom_release_lets_go_cleanly_within_fifty_milliseconds` renders a release and requires no step larger than the held wash's and bit-exact dry output after 50 ms.
