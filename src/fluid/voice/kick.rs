...
pub(crate) struct KickDelay {
    ...
    /// so far has been zero the whole delay line and both filters hold exact
    /// zeros, so `process` can skip all work and return silence unchanged.
    pub(crate) active: bool,
    /// Cached `echo_filter -> lp_coeff` mapping so the `powf` only reruns
    /// when the control value actually changes.
    pub(crate) last_echo_filter: f32,
...
impl KickDelay {
    ...
    pub(crate) fn process(
        ...
        let mut read_pos = self.head + len - delay;
        if read_pos >= len {
            read_pos -= len;
        }
        ...
        if echo_filter != self.last_echo_filter {
            self.last_echo_filter = echo_filter;
            let lp_coeff = 10_f32.powf(echo_filter * 3.6 - 2.3); // ~0.005..2.0 → clamp keeps it stable
            self.lp_coeff = lp_coeff.clamp(0.001, 0.99);
        }
        let lp_coeff = self.lp_coeff;
