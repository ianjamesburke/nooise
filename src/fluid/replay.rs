...
fn production_binding_matrix_crosses_the_complete_pipeline() {
    ...
    let cases = [
        ...
        ("shift max", vec![shift(FixtureKey::Right)]),
        ("L max", vec![shift(FixtureKey::Character('L'))]),
    ...
    for (name, trace) in cases {
        ...
        let expected = match name {
            ...
            "shift max" | "L max" => ExpectedBinding {
                owner: "BROWSE",
                generation: 1,
                automation: None,
                intents: vec![Intent::MaxSelected],
                effects: vec!["MaxSelected=>OK:Published { generation: 1 }"],
                notice: None,
            },
