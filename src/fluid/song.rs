...
/// audible envelope difference. Gestures play over Master only, so the target
/// is always Master's mute bit; an entry aimed at a voice is a retired
/// per-layer gesture and the code is refused.
